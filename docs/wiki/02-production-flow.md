...
`context` step, `SourceIntent` plus `ResearchPlan` in `sourceIntent`, URL retrieval
and search attempts in `publicEvidence`, the quality gate in `feasibility`, the locked `PostContract`, the compiled `RulePack` plus
...
become `SourceIntent`, then a local `ResearchPlan`. `publicEvidence` now reads exact
URLs and can execute OpenRouter `openrouter:web_search` for search/verification
tasks when backend web tools are explicitly enabled. Disabled or unconfigured search
remains visible as `notConfigured`; enriched `SourceLedger` and `EvidenceSynthesis`
remain the next backend layer.
