...
and skipped search tasks in `publicEvidence`, the quality gate in `feasibility`, the locked `PostContract`, the compiled `RulePack` plus
...
URLs and records search tasks as `notConfigured`; enriched `SourceLedger` and
`EvidenceSynthesis` remain the next backend layer.
