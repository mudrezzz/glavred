...
- end the current account session with the always-visible `Выйти` action in the topbar;
...
The dashboard uses the same visual shell as the project cabinet: full-height left
sidebar, topbar, centered working canvas, and the owner profile in the sidebar footer.
In this slice only `Проекты` is active; the `Аккаунт`, `Статистика`, `Биллинг`, and
`Настройки` entries are visible SaaS navigation placeholders for later account
sections. Project cards are shown as a compact tile grid with up to two cards per row
on desktop.

Inside an opened project the lower-left identity block still provides fast project
switching. Use `Все проекты` there to return to the dashboard or `Выйти` to return to
the login screen and switch accounts. Logout remains available when workspace
persistence has fallen back locally, because that state can still have an active
backend cookie.
...
visible. Backend utility scoring shows a categorical recommendation and explains each
...
- Open `Каналы` to configure project-owned publication destinations such as Telegram,
  ...
  string: it has title, handle/URL, language, role, publication mode, status, and an
  optional default publication-size profile. Project audience is configured in the
  `Издательство` rules and can be overridden for a concrete post brief; it is not
  edited on the channel card.
- Open `Матрица`, toggle one compatibility checkbox, then use `Сохранить матрицу` or `Отменить`.
- Open `Сигналы`.
- Inspect `Радары`. In radar settings choose `Языки источников`: only the editorial
  language, editorial language plus English, or any language. This setting controls
  search and source eligibility; the project language controls the editorial wording
  of a signal card.
- Run a radar, then open `Найденные сигналы`. A new extracted signal is shown as
  `На проверке`. Its separate recommendation is `Рекомендуется`, `Проверить с
  осторожностью`, `Не рекомендуется`, or `Недостаточно данных`. Confidence is the
  reliability of extraction from the source, not a score of usefulness for the blog.
- In a structured signal, review `Что утверждает источник` and only the fields that
  apply to its type. A case may have a mechanism and result; a question or tension
  does not receive those fields merely to fill a template. A result is labeled as
  observed, reported, capability-only, expected or not proven. `Открыть источник`
  opens the original page and
  `Показать в трассе` opens the exact RadarRun extraction detail. The source title
  and quotation remain in the original language even when the editorial fields are
  Russian.
- Review `Соответствие радару`, `Соответствие редакционной модели`, `Качество и риски`
  and `Связанные сигналы` separately. Each criterion shows its configured mode,
  user-facing verdict, explanation and readable source quote. Missing proof is not
  automatically a rejection; only a proven conflict with an active blocking criterion
  can produce `Не рекомендуется`.
- `Связанные сигналы` distinguishes one claim, another claim from the same article,
  corroboration and contradiction. Exact/same-claim aliases share one main card, but
  their original evidence remains available. If no relationship proof exists, the UI
  says `Не проверено` rather than showing a default low duplicate risk.
- Approve, archive, reject, correct, reopen, or restore one signal. Rejection, archive,
  and correction require a reason. Correction changes only editorial title, summary,
  and author comment; source evidence remains unchanged and utility is recalculated.
  Approved signal becomes the current raw material for post candidates.
...
- After the revision loop, the trace shows a `Final quality gate`. This gate checks
  the delivered final draft as public prose against an explicit final quality
  contract: whether it leaks internal pipeline terms, reads like a source list, keeps
  source markers, preserves author stance from the editorial model/fabula, and gives
