...
- Run a radar, then open `Найденные сигналы`. A new extracted signal is shown as
  `На проверке` and `Редакционная полезность не оценена` until backend scoring is
  completed. Its confidence is the reliability of extraction from the source, not a
  score of usefulness for the blog.
- In a structured signal, review the summary, mechanism, result, uncertainty,
  limitations, and evidence. `Открыть источник` opens the original page and
