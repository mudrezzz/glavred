...
class DraftMaterialPlanService:
    ...
    def create(
        ...
        *,
        ...
        messages = build_material_plan_messages(context_summary=context_summary, rule_pack=rule_pack)
        ...
        if not status.configured:
            return self._fallback(context_summary, rule_pack, request_payload, provider, model, "OpenRouter is not configured")
        ...
        try:
            ...
            plan = material_plan_from_payload(result.payload)
            ...
            return self._fallback(context_summary, rule_pack, request_payload, AiRunProvider.OPENROUTER, model, self._safe_error(exc))
    ...
    def _fallback(
        ...
        plan = self._deterministic_planning_service.create_material_plan(
            context_summary=context_summary,
            rule_pack=rule_pack,
        )
        payload = plan.to_payload()
