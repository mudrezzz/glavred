...
def create_app(
    ...
    app.add_exception_handler(SqliteStorageError, _sqlite_storage_error_response)
    app.include_router(health_router)
    app.include_router(ai_runs_router)
    app.include_router(drafts_router)
    app.include_router(draft_runs_router)
    app.include_router(radar_runs_router)
    app.include_router(portfolio_router)
    return app


def _sqlite_storage_error_response(_request, exc: SqliteStorageError) -> JSONResponse:
    return JSONResponse(
        status_code=503,
