...
| `application/dossiers` | Semantic inclusion/exclusion policies, typed dossier assembly, planning/writer/alternative-angle/review/ranking/revision/final-quality factories, and provider-free replay. Runtime call sites are live-accepted through `3.9`; the `alternativeAngleRoute` data-dependent cap drift is repaired and live-accepted in `3.9.1`. | provider-heavy prompt builders through `ProviderInputBudgetGate` |
...
16. Clean migrated HITL/provider operation surfaces. Done in Slice 2.17.4.6.0.10;
...
23. Add provider-operation runtime guard coverage for slow provider-heavy calls.
    Planned in Slice 2.17.4.6.1.3.4.
24. Enforce direct provider-input budgets for every provider-heavy child `AiRun`.
...
28. Migrate review, ranking, revision, and final-gate operations to bounded dossiers.
    Done and live-accepted in Slice 2.17.4.6.1.3.9.
29. Repair data-dependent `alternativeAngleRoute` budget drift. Ready in Slice
    2.17.4.6.1.3.9.1.
30. Pilot tool-mediated context access over the deterministic context service.
    Planned in Slice 2.17.4.6.1.3.10.
...
- Remaining provider-heavy operations stay explicit payload-budget debt entries in
  ...
  boundary at runtime.
- Slice 2.17.4.6.1.3.5 adds `ProviderInputBudgetGate` and replayable
  `ProviderInputAudit`. `materialPlan`, `strategy`, and `rhetoricalPlans` now write
  direct child `AiRun.requestPayload.operationId`, `providerInput`, `payloadBudget`,
  `inputStats`, and `payloadStats`; `strategy` keeps the existing budget profile
  `draftStrategy` and records `operationAlias=strategy`.

The provider-input treatment track after Slice 2.17.4.6.1.3.3 tightens this rule:
the budget must belong to the current provider call. A nested `payloadBudget` from
an earlier artifact is not proof that the current prompt was compacted. The target
architecture is:

`DraftRun artifacts -> DraftRunContextAccessService -> DossierFactory -> ProviderInputBudgetGate -> PromptBuilder -> Provider`

The rich artifacts stay in parent DraftRun storage. Provider calls receive typed
planning, writer, review, ranking, revision, or final-quality dossiers plus handles
...
- Slice `2.17.4.6.1.3.9.1` adds `AlternativeAngleRoutePayloadCompactor` and
  `AlternativeAngleRouteProjection` for equal bounded candidate sections, plus
  `ProviderMessageBudgetGuard` for the final serialized message cap;
