...
class FinalQualityAttributionPolicy:
    """Owns actionable-vs-diagnostic attribution finding classification."""

    def effective_finding_counts(self, report: dict[str, Any]) -> dict[str, int]:
        actionable = [_dict(item) for item in _list(report.get("findings")) if self.is_actionable_finding(_dict(item))]
        return {
    ...
    def attribution_finding_split(self, report: dict[str, Any]) -> dict[str, list[dict[str, Any]]]:
        ...
        for raw in _list(report.get("findings")):
            ...
            if not validator.startswith("evidence.attribution"):
                continue
            if self.is_actionable_finding(finding):
                ...
                diagnostic.append({
                    **finding,
                    "metadata": {
                        **metadata,
                        "suppressedReason": metadata.get("suppressedReason") or self.suppressed_reason(finding),
                    },
                })
        return {"actionableAttributionFindings": actionable, "diagnosticAttributionNoise": diagnostic}

    def is_actionable_finding(self, finding: dict[str, Any]) -> bool:
        validator = str(finding.get("validatorId") or "")
    ...
    def suppressed_reason(self, finding: dict[str, Any]) -> str:
        metadata = _dict(finding.get("metadata"))
