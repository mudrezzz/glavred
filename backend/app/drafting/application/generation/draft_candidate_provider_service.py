...
from backend.app.drafting.application.generation.draft_candidate_audit import build_candidate_request_trace, build_candidate_result_trace
from backend.app.drafting.application.generation.draft_candidate_prompts import CANDIDATE_KEYS, build_draft_candidate_messages
...
class DraftCandidateProviderService:
    ...
    def create_one(
        ...
        *,
        ...
        if not status.configured:
            request_payload = self._request_payload(provider, primary_selection.model, context_summary, rule_pack, material_plan, draft_strategy, direction, context_pack, primary_selection.to_payload())
        ...
        for attempt in build_json_step_attempts(primary_model=primary_selection.model or self._settings.openrouter_default_model, backup_model=self._settings.openrouter_backup_model_or_none):
            ...
            if result["accepted"]:
                return result["candidate"], _ai_run_ids(attempts)
        ...
        request_payload = self._request_payload(AiRunProvider.OPENROUTER, primary_selection.model, context_summary, rule_pack, material_plan, draft_strategy, direction, context_pack, primary_selection.to_payload())
        payload, ai_run_id = self._fallback(request, direction, rule_pack, material_plan, {**request_payload, "attempts": attempts}, AiRunProvider.OPENROUTER, primary_selection.model, _last_error(attempts) or "Draft candidate JSON generation failed after all attempts")
        return payload, [*_ai_run_ids(attempts), *([ai_run_id] if ai_run_id else [])]
    ...
    def _attempt_error(self, attempt: JsonStepAttempt, request_payload: dict[str, Any], error: str, raw_response_excerpt: str | None = None) -> dict[str, Any]:
        ...
        return {"accepted": False, "candidate": None, "attempt": _attempt_record(attempt, run.id, "error", selection, error)}
