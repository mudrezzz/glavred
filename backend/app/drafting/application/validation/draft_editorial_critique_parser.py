...
def parse_editorial_critique(
    *,
    candidate_id: str,
    payload: dict[str, Any],
) -> tuple[list[DraftValidatorFinding], list[EditorialCritiqueObservation]]:
    findings = [_finding(candidate_id, _dict(item)) for item in _list(payload.get("findings"))]
    observations = [_observation(candidate_id, _dict(item)) for item in _list(payload.get("observations"))]
    return findings, observations


def critique_dimension(value: Any) -> str:
    text = str(value or "").strip()
    return text if text in _DIMENSIONS else "ideaStrength"


def editorial_risk(value: Any) -> str:
    text = str(value or "").strip().lower()
    return text if text in {"low", "medium", "high", "unknown"} else "unknown"


def _finding(candidate_id: str, raw: dict[str, Any]) -> DraftValidatorFinding:
    dimension = critique_dimension(raw.get("editorialDimension") or raw.get("dimension"))
...
def _observation(candidate_id: str, raw: dict[str, Any]) -> EditorialCritiqueObservation:
    dimension = critique_dimension(raw.get("editorialDimension") or raw.get("dimension"))
