...
from backend.app.drafting.application.validation.draft_llm_validation_observations import (
    LlmValidationObservationNormalizer,
)
...
class LlmValidationParser:
    def __init__(self, observations: LlmValidationObservationNormalizer | None = None) -> None:
        self._observations = observations or LlmValidationObservationNormalizer()
    ...
    def parse_report(
        ...
        *,
        ...
        for item in _list(payload.get("findings")):
            raw = _dict(item)
            if self._observations.is_observation_like(raw):
                observations.append(self._observations.observation(candidate_id=candidate_id, payload=payload, raw=raw))
            else:
                findings.append(self._finding(candidate_id=candidate_id, payload=payload, raw=raw))
        for item in _list(payload.get("observations")):
            observations.append(self._observations.observation(candidate_id=candidate_id, payload=payload, raw=_dict(item)))
        return findings, observations

    def parse_findings(self, *, candidate_id: str, payload: dict[str, Any]) -> list[DraftValidatorFinding]:
        return self.parse_report(candidate_id=candidate_id, payload=payload)[0]

    def status_for_payload(self, *, candidate_id: str, payload: dict[str, Any]) -> DraftValidatorStatus:
        return validation_status_for(self.parse_findings(candidate_id=candidate_id, payload=payload))

    def _finding(self, *, candidate_id: str, payload: dict[str, Any], raw: dict[str, Any]) -> DraftValidatorFinding:
        return DraftValidatorFinding(
            validator_id=self._observations.normalize_validator_id(raw),
