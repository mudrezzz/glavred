...
Used by: provider attempt builders that must prove the final serialized message size.
Does not own: provider-input compaction, prompt wording, or provider transport.
...
class ProviderMessageBudgetGuard:
    ...
    def evaluate(
        ...
        *,
        ...
        provider_input_chars = int(payload_budget.get("promptCharEstimate") or 0)
        message_chars = len(json.dumps(messages, ensure_ascii=False, sort_keys=True))
        max_chars = int(dict(payload_budget.get("limits") or {}).get("maxPromptChars") or 0)
