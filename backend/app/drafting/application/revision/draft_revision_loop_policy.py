...
class DraftRevisionLoopPolicy:
    """Owns deterministic revision-loop stop, constraint, and failure payload policy."""

    combined_validation = staticmethod(combined_validation)
    pairwise_winner = staticmethod(pairwise_winner)
    failed_cycle = staticmethod(failed_cycle)
    constraints_from_unresolved = staticmethod(constraints_from_unresolved)
    constraints_from_rejection = staticmethod(constraints_from_rejection)
    candidate_id = staticmethod(candidate_id)
    dict_or_empty = staticmethod(dict_or_empty)
    string_list = staticmethod(string_list)
    last_value = staticmethod(last_value)
