...
def constraints_from_rejected_moves(moves: list[dict[str, Any]]) -> list[str]:
...
def constraints_from_editorial_goals(goals: list[dict[str, Any]]) -> list[str]:
    return [f"Still unresolved editorial goal: {message}" for message in goal_messages(goals[:6])]


def goal_messages(goals: list[dict[str, Any]]) -> list[str]:
    return [str(goal.get("message") or goal.get("id")) for goal in goals if str(goal.get("message") or goal.get("id") or "").strip()]


def cycle_stop_reason(accepted: bool, unresolved_goals: list[str], unresolved_editorial_goals: list[dict[str, Any]]) -> str:
    if not accepted:
...
class DraftRevisionRejectedMovesPolicy:
    """Owns rejected-move extraction and anti-regression constraints for revision cycles."""

    rejected_moves_from_cycle = staticmethod(rejected_moves_from_cycle)
    constraints_from_rejected_moves = staticmethod(constraints_from_rejected_moves)
    constraints_from_editorial_goals = staticmethod(constraints_from_editorial_goals)
    goal_messages = staticmethod(goal_messages)
    cycle_stop_reason = staticmethod(cycle_stop_reason)
