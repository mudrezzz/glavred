...
class RankingRevisionCandidateMapper:
    """Owns mapping helpers between ranking/revision payloads and draft DTOs."""

    def candidate_by_id(self, draft_artifact: dict[str, Any], candidate_id: str | None) -> dict[str, Any] | None:
        for candidate in draft_artifact.get("candidates", []):
            if isinstance(candidate, dict) and candidate.get("id") == candidate_id:
                return candidate
        return None

    def candidate_to_draft(self, request: DraftGenerationRequest, candidate: dict[str, Any]) -> GeneratedDraft:
    ...
    def last_revised(self, loop_payload: dict[str, Any]) -> dict[str, Any] | None:
    ...
    def final_reason(self, source: str, ranking_reason: str, stop_reason: str) -> str:
