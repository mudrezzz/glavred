...
const proofDir = path.join(rootDir, 'var', 'visual-proof', '2.17.4.7.0.1', 'live');
...
async function openIndustrialRadar(page) {
  ...
  const radar = page.locator('[data-testid="radar-row"]').filter({ hasText: 'Industrial AI cases' }).first();
  await radar.waitFor();
...
try {
  ...
  await radar.getByRole('link', { name: 'Открыть трассу' }).click();
  await page.waitForURL(/\/radar-runs\?runId=/u);
